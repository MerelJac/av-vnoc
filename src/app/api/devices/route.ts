...
import { getAccessibleCustomerIds, deviceTenancyWhere } from "@/lib/tenancy";
...
export async function GET(req: NextRequest) {
  ...
  const where = {
    ...
    ...(!unassigned && customerId
      ...
      : {}),
    // Tenancy scoping goes under AND so an explicit ?customerId= filter can
    // never widen access beyond the user's assigned customers.
    ...(accessibleCustomerIds ? { AND: [deviceTenancyWhere(accessibleCustomerIds)] } : {}),
  };
